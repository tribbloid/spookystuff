...
package org.apache.spark.benchmark.__inline
...
/**
  * Utility class to benchmark components. An example of how to use this is: val benchmark = new Benchmark("My
  * Benchmark", valuesPerIteration) benchmark.addCase("V1")(<function>) benchmark.addCase("V2")(<function>)
  * benchmark.run This will output the average time to run each function and the rate of each function.
  *
  * The benchmark function takes one argument that is the iteration that's being run.
  *
  * @param name
  *   name of this benchmark.
  * @param valuesPerIteration
  *   number of values used in the test case, used to compute rows/s.
  * @param minNumIters
  *   the min number of iterations that will be run per case, not counting warm-up.
  * @param warmupTime
  *   amount of time to spend running dummy case iterations for JIT warm-up.
  * @param minTime
  *   further iterations will be run for each case until this time is used up.
  * @param outputPerIteration
  *   if true, the timing for each run will be printed to stdout.
  * @param output
  *   optional output stream to write benchmark results to
  */
private[spark] class Benchmark(
  ...
  /**
    * Adds a case to run when run() is called. The given function will be run for several iterations to collect timing
    * statistics.
    *
    * @param name
    *   of the benchmark case
    * @param numIters
    *   if non-zero, forces exactly this many iterations to be run
    */
  ...
  /**
    * Adds a case with manual timing control. When the function is run, timing does not start until timer.startTiming()
    * is called within the given function. The corresponding timer.stopTiming() method must be called before the
    * function returns.
    *
    * @param name
    *   of the benchmark case
  ...
  /**
    * Runs the benchmark and outputs the results to stdout. This should be copied and added as a comment with the
    * benchmark. Although the results vary from machine to machine, it should provide some baseline.
    */
  def run(): Unit = {
  ...
  /**
    * Runs a single function `f` for iters, returning the average time the function took and the rate of the function.
    */
...
private[spark] object Benchmark {
  ...
  /**
    ...
    *
    * @param iteration
    *   specifies this is the nth iteration of running the benchmark case
    */
  ...
  /**
    * This should return a user helpful processor information. Getting at this depends on the OS. This should return
    * something like "Intel(R) Core(TM) i7-4870HQ CPU @ 2.50GHz"
    */
  def getProcessorName(): String = {
    val cpu = if (SystemUtils.IS_OS_MAC_OSX) {
      Utils.executeAndGetOutput(Seq("/usr/sbin/sysctl", "-n", "machdep.cpu.brand_string")).stripLineEnd
  ...
  /**
    * This should return a user helpful JVM & OS information. This should return something like "OpenJDK 64-Bit Server
    * VM 1.8.0_65-b17 on Linux 4.1.13-100.fc21.x86_64"
    */
