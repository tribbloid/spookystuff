...
case class MAVLink(
  ...
  override protected def detectConflicts(): Unit = {
    ...
    val conflicting = drivers.filter { driver =>
      driver.historyCodeOpt.exists(_.contains(uri))
    }
    val myDriver = Option(Endpoints.primary._driver)
    val notMyDriver: Seq[PythonDriver] = conflicting.filterNot { driver =>
  ...
  object synchAPI extends SynchronousAPI {
    ...
    override def testMove: String = {
      Endpoints.primary.PY.testMove().$STR.get
    }
