...
case class Location(
  ...
  @transient lazy val reanchorToPrimary: this.type = {
    val children = definedBy.flatMap { v =>
      v.children
    }
  ...
  def simpleString: String = {
    name + ": " + reanchorToPrimary.definedBy.headOption
      .map(_.simpleString)
      .mkString("<", "", ">")
  }
