...
case class InsertWP(
  ...
  case class Inst(gp: TrafficControl, schema: SpookySchema) extends ResamplerInst {
    ...
    def apply(v: Map[Int, Seq[Trace]]): Map[Int, Seq[Trace]] = {
      ...
      val indexed: Map[Int, Seq[List[(Action, (Int, Int))]]] = v.mapValues {
        seq =>
      ...
      val resamplingBuffers: Map[Int, Seq[ResamplingBuffer]] = indexed.mapValues {
        ss =>
          val navs = ss.flatten.collect {
            case (nav: UAVNavigation, i) => nav.WSchema(schema) -> i
          }
          val result = navs.indices.dropRight(1).map {
            i =>
      ...
      val replacements: Map[Int, Map[(Int, Int), Seq[UAVNavigation]]] = resamplingBuffers.mapValues {
        seq =>
          val list = seq.map {
            v =>
              val index = v.to._2
