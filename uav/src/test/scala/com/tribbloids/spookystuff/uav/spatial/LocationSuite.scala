...
class LocationSuite extends FunSpecx {
  ...
  it("Location can infer LLA from NED") {
    ...
    val p1 : Location = LLA.vec2Coordinate(Vec(-79.262262, 43.694195, 136)) -> Anchors.Geodetic

    val p2 : Location = NED.vec2Coordinate(Vec(1000, 2000, 30)) -> p1
  ...
  it("Location can infer NED from LLA") {
    ...
    val p2 : Location = LLA.vec2Coordinate(Vec(-79.386132, 43.647023, 100)) -> Anchors.Geodetic
  ...
  it("Location can handle cyclic referential NED") {
    ...
    p1._cache(NED.vec2Coordinate(Vec(-1000,-2000,-30)) -> p2)
  ...
  it("Location can handle cyclic referential LLA") {
    ...
    val p2 : Location = LLA.vec2Coordinate(Vec(-79.262262, 43.694195, 30)) -> p1

    p1._cache(LLA.vec2Coordinate(Vec(-79.262262, 43.694195,-30)) -> p2)
