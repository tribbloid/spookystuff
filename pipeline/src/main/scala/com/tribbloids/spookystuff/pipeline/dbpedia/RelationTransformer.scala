...
class RelationTransformer(
  ...
  final val MaxDepth: Param[Int] = new Param(this, "MaxDepth", "depth of traversing")
  final val DepthCol: Param[Symbol] = new Param(this, "DepthKey", "column that contains Depth of relations")
  final val OutputURICol: Param[Symbol] = new Param[Symbol](this, "OutputURICol", "column that contains URI of output entities")

  setExample(InputURICol -> '_, Relation -> ".*appointer", ReverseRelation -> ".*appointer", MaxDepth -> 2, DepthCol -> 'depth, OutputURICol -> 'uri, FirstN -> 3)
  setDefault(MaxDepth -> 1, DepthCol -> null, OutputURICol -> null, FirstN -> Int.MaxValue)
  ...
  override def transform(dataset: PageRowRDD): PageRowRDD = {
    ...
    dataset
      ...
      .wgetExplore(
        ...
        maxDepth = getOrDefault(MaxDepth),
        depthKey = getOrDefault(DepthCol),
        failSafe = 2,
