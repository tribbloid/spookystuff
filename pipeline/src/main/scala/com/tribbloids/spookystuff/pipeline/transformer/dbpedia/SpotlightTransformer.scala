...
class SpotlightTransformer(
                               ...
                               classOf[SpotlightTransformer].getCanonicalName + "_" + UUID.randomUUID().toString
  ...
  //TODO: simplify by specifying an example for each Param, then use it and assert none of the columns is NULL
  def test(spooky: SpookyContext): Unit = {
    ...
    val transformer = new SpotlightTransformer()
      .setInputCol("input")
      .setMaxPages(2)
      .setPageNumCol("page")
      .setIndexCol("index")
