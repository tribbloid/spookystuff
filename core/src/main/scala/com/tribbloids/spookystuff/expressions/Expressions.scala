...
import com.tribbloids.spookystuff.row.{DataRow, Field, PageRow}
...
final case class Literal[+T: ClassTag](value: T) extends LiftedExpression[T] {
  ...
  def liftApply(v1: PageRow): Option[T] = Some(value)
}

case object NullLiteral extends LiftedExpression[Null]{

  def liftApply(v1: PageRow): Option[Null] = None
}

class GetExpr(val field: Field) extends LiftedExpression[Any] {

  def liftApply(v1: PageRow): Option[Any] = {
...
object GroupIndexExpr extends LiftedExpression[Int] {
  ...
  def liftApply(v1: PageRow): Option[Int] = Some(v1.dataRow.groupIndex)
}

class GetUnstructuredExpr(val field: Field) extends LiftedExpression[Unstructured] {

  def liftApply(v1: PageRow): Option[Unstructured] = {
    v1.getUnstructured(field)
      .orElse(v1.getUnstructured(field.copy(isWeak = true)))
  }
}

class GetPageExpr(val field: Field) extends LiftedExpression[Page] {

  def liftApply(v1: PageRow): Option[Page] = v1.getPage(field.name)
}

class FindFirstExpr(selector: String, param: Expression[Unstructured]) extends LiftedExpression[Unstructured] {

  def liftApply(v1: PageRow): Option[Unstructured] = param.lift(v1).flatMap(_.findFirst(selector))

  def expand(range: Range) = new LiftedExpression[Siblings[Unstructured]] {

    def liftApply(v1: PageRow) = param.lift(v1).flatMap(_.findFirstWithSiblings(selector, range))
  }
}

class FindAllExpr(selector: String, param: Expression[Unstructured]) extends LiftedExpression[Elements[Unstructured]] {

  def liftApply(v1: PageRow): Option[Elements[Unstructured]] = param.lift(v1).map(_.findAll(selector))

  def expand(range: Range) = new LiftedExpression[Elements[Siblings[Unstructured]]] {

    def liftApply(v1: PageRow) = param.lift(v1).map(_.findAllWithSiblings(selector, range))
  }
}

class ChildExpr(selector: String, param: Expression[Unstructured]) extends LiftedExpression[Unstructured] {

  def liftApply(v1: PageRow): Option[Unstructured] = param.lift(v1).flatMap(_.child(selector))

  def expand(range: Range) = new LiftedExpression[Siblings[Unstructured]] {

    def liftApply(v1: PageRow) = param.lift(v1).flatMap(_.childWithSiblings(selector, range))
  }
}

class ChildrenExpr(selector: String, param: Expression[Unstructured]) extends LiftedExpression[Elements[Unstructured]] {

  def liftApply(v1: PageRow): Option[Elements[Unstructured]] = param.lift(v1).map(_.children(selector))

  def expand(range: Range) = new LiftedExpression[Elements[Siblings[Unstructured]]] {

    def liftApply(v1: PageRow) = param.lift(v1).map(_.childrenWithSiblings(selector, range))
  }
}

class GetSeqExpr(val field: Field) extends LiftedExpression[Seq[Any]] {

  def liftApply(v1: PageRow): Option[Seq[Any]] = v1.dataRow.get(field).flatMap {
...
object GetOnlyPageExpr extends LiftedExpression[Page] {

  def liftApply(v1: PageRow): Option[Page] = v1.getOnlyPage
}

object GetAllPagesExpr extends LiftedExpression[Elements[Page]] {

  def liftApply(v1: PageRow): Option[Elements[Page]] = Some(new Elements(v1.pages.toList))
}

class ReplaceKeyExpr(str: String) extends LiftedExpression[String] {

  def liftApply(v1: PageRow): Option[String] = v1.dataRow.replaceInto(str)
}
...
class InterpolateExpr(parts: Seq[String], fs: Seq[Expression[Any]]) extends LiftedExpression[String] {
  ...
  def liftApply(v1: PageRow): Option[String] = {
...
class ZippedExpr[T1,+T2](param1: Expression[IterableLike[T1, _]], param2: Expression[IterableLike[T2, _]]) extends LiftedExpression[Map[T1, T2]] {
  ...
  def liftApply(v1: PageRow): Option[Map[T1, T2]] = {
