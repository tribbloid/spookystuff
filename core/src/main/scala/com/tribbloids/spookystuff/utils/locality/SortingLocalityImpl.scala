...
case class SortingLocalityImpl[K: ClassTag, V: ClassTag](
  ...
  {
    assert(rdd1.partitioner.nonEmpty,
      s"cannot use ${this.getClass.getCanonicalName} on RDD without partitioner")
  }
  ...
  override def cogroupBase[V2: ClassTag](rdd2: RDD[(K, V2)]): RDD[(K, (V, Iterable[V2]))] = {
    ...
    val flatten: RDD[(K, (Long, V, Iterable[V2]))] = cogrouped.flatMapValues {
      tuple =>
        tuple._1.map {
          t =>
            (t._2, t._1, tuple._2)
    ...
    val result = flatten
      .mapPartitions {
        itr =>
          itr.toSeq.sortBy(_._2._1).iterator //TODO: may cause OOM! optimize later
            .map {
            case (k, v) =>
              k -> (v._2 -> v._3)
          }
