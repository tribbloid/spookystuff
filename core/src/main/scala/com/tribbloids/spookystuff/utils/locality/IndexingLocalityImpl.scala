...
case class MapPartitioner(
  ...
  override def getPartition(key: Any) = {
    key2PartitionID_broadcasted.value.getOrElse(key,
...
case class IndexingLocalityImpl[K: ClassTag, V: ClassTag](
  ...
  lazy val beacon: RDD[(Int, (K, V))] = {
    val withPID = rdd1.mapPartitions {
      itr =>
  ...
  override def cogroupBase[V2: ClassTag](rdd2: RDD[(K, V2)]) = {
    ...
    val aligned = beacon.zipPartitions(rdd2Repartitioned) {
      (beaconPartition, partition2) =>
        ...
        val joinedPartition = partition1.map {
          tuple =>
