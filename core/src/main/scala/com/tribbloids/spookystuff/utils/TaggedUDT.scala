...
/**
  ...
  */
class TaggedUDT[T](@transient implicit val ttg: TypeTag[T]) extends UserDefinedType[T] with IdentifierMixin {

  import org.apache.spark.sql.TypeUtils.Implicits._

  val ctg: ClassTag[T] = ttg.toClassTag
...
//  override def sqlType: DataType = this.getClass.getConstructor().newInstance()
  ...
  override def _id: Any = userClass.getCanonicalName
}